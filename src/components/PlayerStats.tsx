...
import { Heart, Sword, Shield, MapPin, Coins, Gem, TrendingUp } from 'lucide-react';
...
export const PlayerStats: React.FC<PlayerStatsProps> = ({ playerStats, zone, coins, gems, multipliers }) => {
  return (
    <div className="bg-gradient-to-br from-slate-800/80 via-slate-900/80 to-black/80 p-4 sm:p-6 rounded-lg shadow-2xl border border-slate-600">
      ...
      <div className="space-y-3 sm:space-y-4">
        ...
        <div className="bg-black/30 p-3 rounded-lg">
          <div className="flex items-center gap-2 mb-2">
            ...
            {multipliers.hp > 1 && (
              <span className="text-green-400 text-xs">({multipliers.hp.toFixed(1)}x)</span>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          <div className="bg-black/30 p-2 sm:p-3 rounded-lg">
            <div className="flex items-center gap-1 sm:gap-2 mb-1">
              ...
              {multipliers.atk > 1 && (
                <span className="text-green-400 text-xs">({multipliers.atk.toFixed(1)}x)</span>
              )}
            </div>
          ...
          <div className="bg-black/30 p-2 sm:p-3 rounded-lg">
            <div className="flex items-center gap-1 sm:gap-2 mb-1">
              ...
              {multipliers.def > 1 && (
                <span className="text-green-400 text-xs">({multipliers.def.toFixed(1)}x)</span>
              )}
            </div>
        ...
        <div className="grid grid-cols-2 gap-2 sm:gap-3">
          <div className="bg-black/30 p-2 sm:p-3 rounded-lg">
            <div className="flex items-center gap-1 sm:gap-2 mb-1">
              ...
              {multipliers.coins > 1 && (
                <span className="text-green-400 text-xs">({multipliers.coins.toFixed(1)}x)</span>
              )}
            </div>
          ...
          <div className="bg-black/30 p-2 sm:p-3 rounded-lg">
            <div className="flex items-center gap-1 sm:gap-2 mb-1">
              ...
              {multipliers.gems > 1 && (
                <span className="text-green-400 text-xs">({multipliers.gems.toFixed(1)}x)</span>
              )}
            </div>
        ...
        {/* Multipliers Summary */}
        {(multipliers.coins > 1 || multipliers.gems > 1 || multipliers.atk > 1 || multipliers.def > 1 || multipliers.hp > 1) && (
          <div className="bg-gradient-to-r from-green-900/30 to-emerald-900/30 p-3 rounded-lg border border-green-500/30">
            <div className="flex items-center gap-2 mb-2">
              <TrendingUp className="w-4 h-4 text-green-400" />
              <span className="text-green-400 font-semibold text-sm">Active Multipliers</span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-xs">
              {multipliers.coins > 1 && (
                <span className="text-yellow-300">Coins: {multipliers.coins.toFixed(1)}x</span>
              )}
              {multipliers.gems > 1 && (
                <span className="text-purple-300">Gems: {multipliers.gems.toFixed(1)}x</span>
              )}
              {multipliers.atk > 1 && (
                <span className="text-orange-300">ATK: {multipliers.atk.toFixed(1)}x</span>
              )}
              {multipliers.def > 1 && (
                <span className="text-blue-300">DEF: {multipliers.def.toFixed(1)}x</span>
              )}
              {multipliers.hp > 1 && (
                <span className="text-red-300">HP: {multipliers.hp.toFixed(1)}x</span>
              )}
            </div>
          </div>
        )}
